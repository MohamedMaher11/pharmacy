...
class _DoctorProfilePageState extends State<DoctorProfilePage> {
              ...
              children: [
                Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.center,
                    children: [
                      SizedBox(height: 60), // لتعديل المسافة بين الصورة والنصوص
                      Text(
                        widget.doctor['name'],
                        style: TextStyle(
                          fontSize: 28,
                      ...
                      SizedBox(height: 20),
                      _buildConsultationFeeContainer(),
                      SizedBox(height: 20),
                      _buildRatingAndReviewForm(),
                      SizedBox(height: 20),
                      buildReviewsSection(doctor: widget.doctor.id),
                    ],
                  ),
                ),
                Positioned(
  ...
  Widget _buildConsultationFeeContainer() {
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            'سعرالحجز  ${widget.doctor['consultationFee']} جنيه',
            style: TextStyle(
              fontSize: 18,
              fontWeight: FontWeight.bold,
              color: Colors.deepPurple,
  ...
  void _submitReview() async {
    ...
    // تحقق من هوية المستخدم (دكتور أو مستخدم عادي)
    ...
    final reviewData = {
      ...
      'userImageUrl': userData['imageUrl'], // صورة المستخدم
      'timestamp': Timestamp.now(),
    };

    print('Review Data: $reviewData'); // طباعة بيانات التقييم للتأكد
    ...
    try {
      ...
      showAlertDialog(context, 'Review submitted successfully!',
          isSuccess: true);
    } catch (e) {
      setState(() {
        _isLoading = false;
      });
      showAlertDialog(context, 'Failed to submit review: $e', isError: true);
    }
  }
